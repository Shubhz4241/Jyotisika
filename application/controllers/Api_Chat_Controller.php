...
class Api_Chat_Controller extends CI_Controller {
    ...
    public function Start_session() {
        ...
        if (empty($data['user_id']) || !$astrologer_id) {
    ...
    public function Get_chat_history($user_id) {
        ...
        http_response_code(200);
        echo json_encode(['status' => true, 'message' => 'Chat history retrieved', 'data' => $history]);
    }
    ...
    public function End_session() {
        ...
        if (empty($data['session_id'])) {
            http_response_code(400);
            echo json_encode(['status' => false, 'message' => 'Invalid session ID']);
            return;
        }
    ...
    public function Get_users() {
        ...
        http_response_code(200);
        echo json_encode(['status' => true, 'message' => 'Users fetched successfully', 'data' => $users]);
    }
    ...
    public function Add_income() {
        ...
        if (!$astrologer_id) {
            return $this->output->set_content_type('application/json')
                                ...
                                ->set_output(json_encode(["status" => false, "message" => "Astrologer not logged in"]));
        }
