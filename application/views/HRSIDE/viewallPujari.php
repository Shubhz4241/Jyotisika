...
<body style="background-color:rgb(228, 236, 241);">
    <div class="d-flex">
        ...
        <div class="main w-100">
            ...
            <div class="container">
                ...
                <a href="<?php echo site_url('HRAdmin/ViewpujariProfile'); ?>" class="text-decoration-none text-dark">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            fetch("<?php echo base_url('admin/getpujarisByStatus?status=Approved'); ?>")
                .then(response => response.json())
                .then(data => {
                    if (data.status === "success") {
                        let pujaris = data.data;
                        let pujariContainer = document.getElementById("pujari-list");

                        pujariContainer.innerHTML = ""; // Clear existing content
                        ...
                        <a href="${profileLink}" class="profile-link">
                            <div class="profile-card">
                                <img src="${profileImage}" alt="${pujari.name}">
                                <div class="name pujari">${pujari.name}</div>
                            </div>
                        </a>
                    `;
                            pujariContainer.innerHTML += card;
                        });
                    } else {
                        document.getElementById("pujari-list").innerHTML = "<p>No pujaris found.</p>";
                    }
                })
                .catch(error => console.error("Error fetching pujaris:", error));
        });
    </script>
