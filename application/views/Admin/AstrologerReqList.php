...
<body style="background-color: rgb(228, 236, 241);">
        ...
        <div class="container">
          ...
          <div class="float-end">
            <ul class="nav nav-pills mb-3">
              ...
              <li class="nav-item"><button class="nav-link" onclick="filterTable('pending')">Scheduled</button></li>
          ...
          <div class="tab-pane fade show active" id="astrologerTable">
  ...
  <script>
    let currentPage = 1;
    const rowsPerPage = 7;
    const baseImagePath = "<?= base_url('uploads/') ?>";
    const defaultImage = "<?= base_url('assets/images/HRside/Profile1.png') ?>";

    let AstrologersData = <?= json_encode($Astro) ?>;
    let filteredData = [];
    ...
    function filterTable(status) {
      ...
      //   filteredData = AstrologersData;
      // } else {
      //   filteredData = AstrologersData.filter(item => (item.status || '').toLowerCase() === key);
      // }
      if (status.toLowerCase() === 'newest') {
  filteredData = AstrologersData.filter(item => (item.status || '').toLowerCase() === 'pending');
} else {
  filteredData = AstrologersData.filter(item => (item.status || '').toLowerCase() === key);
}
    ...
    function displayData(page) {
      ...
      if (paginated.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="10" class="text-center text-muted">No Astrologers found.</td></tr>`;
        updatePagination();
        return;
      }
          ...
          <tr>
            ...
            <td class="text-center">
              ${isRejected 
                ...
                : `<button class="btn btn-sm btn-primary" onclick="window.location.href='viewastrologere/${item.id}'">View</button>`}
            </td>
          </tr>
        `;
